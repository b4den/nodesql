...
var { 
  ...
  get_tables,
  get_columns,
  get_dump } = require(__dirname + '/../models/injection_model');
...
module.exports = (router) => {
  ...
  router.post('/api/database', async (req, res) => { 
    ...
    try {
      let db_info = await get_databases(url)
  ...
  router.post('/api/tables', async (req, res) => { 
    ...
    try {
      let db_info = await get_tables(url, database)
      res.send({"data":db_info})
    }
    catch (err) {
      console.log(err)
  ...
  router.post('/api/columns', async (req, res) => { 
    ...
    try {
      let db_info = await get_columns(url, database, table)
      res.send({"data":db_info})
    }
    catch (err) {
      console.log(err)
  ...
  router.post('/api/dump', async (req, res) => { 
    ...
    try {
      let db_info = await get_dump(url, database, table)
      res.send({"data":db_info})
    }
    catch (err) {
      console.log(err)
